...
def create_app():
    ...
    file_handler = RotatingFileHandler(
        "logs/caiyuan.log", maxBytes=102400, backupCount=10
    )
    ...
    def favicon():
        # Serve generated favicon.ico from static if present, otherwise fall back to static JPG
        static_dir = os.path.join(app.root_path, "static")
        ico_path = os.path.join(static_dir, "favicon.ico")
        if os.path.exists(ico_path):
            return send_from_directory(static_dir, "favicon.ico")
        # fallback to static JPG
        return send_from_directory(static_dir, "caiyun_icon.jpg")
